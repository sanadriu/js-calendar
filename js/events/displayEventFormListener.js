...
export default function displayEventFormListener() {
	document.addEventListener("click", (e) => {
		...
		if (e.target.matches("[data-action~='set-cell-date']")) return displayEventForm(sessionStorage.calendarYear, sessionStorage.calendarMonth, parseInt(e.target.dataset.day));
		if (e.target.matches("[data-action~='set-current-date']")) return displayEventForm();
	});
}
